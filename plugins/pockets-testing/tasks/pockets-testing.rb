...
def generate_sources(opts = {})
  sources_yaml = YAML.load_file('pockets/sources.yaml') if File.exists?('pockets/sources.yaml')
  load_first = sources_yaml ? sources_yaml['load_first'] : []
  ...
  if (opts[:test])
    test_load_first = sources_yaml['test'] ? sources_yaml['test']['load_first'] : []
